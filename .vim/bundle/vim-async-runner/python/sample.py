...
def project_directory_parts(full_file_path = None):
...
def create_test_command(directory_path, test_class, test_method):
    file_parts, local_parts, file_name = project_directory_parts((current_full_file_path()))

    if directory_path.endswith(".py"):
        file_name_without_extension = re.sub('\.py$', '', file_name)
        python_exe = "/".join(file_parts+ ['env', 'bin', 'python'])
        test_parts = local_parts + [file_name_without_extension] + [test_class, test_method]
        test_parts = filter(lambda p: p is not None, test_parts )

        return python_exe + " -m unittest " +  ".".join(test_parts)
...
def save_async_command(command, directory):
    print(f"saving command to run later ({command})")
    command_info = {
        'command': command,
        'directory': directory
    }
    save_in_variable("vimasyncrunnercommand", json.dumps(command_info))

def run_async_command():
    command_info = json.loads(vim.eval('g:vimasyncrunnercommand'))
    print(f"sending to async runner ({command_info['command']}) ")
    sample.run_command(command_info['command'], command_info['directory'])

def save_in_variable(var_name, text_to_save):
    vim.command(f"let g:{var_name}= '{text_to_save}'")
